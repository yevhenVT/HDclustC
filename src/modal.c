...
// Finds same state sequence as entry in mat array
...
void SortDouble(double *org, double *buf, int *invid, int sz)
{
  ...
  if (score==NULL) {
    fprintf(stderr, "Unable to allocate space in SortDouble.\n");
    exit(0);
  }
...
{
  int i,j,k,m,n, ite, minite=30, maxite=1000, maxout=50;
  ...
  while (ite<maxite) {
    ...
    // estimate distance between oldmode and newmode
