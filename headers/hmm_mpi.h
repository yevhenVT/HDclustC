...
// old value of EPSILON is 1.0e-5
...
typedef struct condchain_struct
{
  ...
  int *cbdim; //cumulated starting position of the dimension of each block [nb]
...
/*------------- utils_mpi.c -------------*/
/*---------------------------------------*/
extern void send_new_ccm(CondChain *md); // send CondChain model from  master to slaves. It is 
										 // assumed that memory for chain structure is not allocated on slaves

extern void send_new_hhm(HmmModel *md);  // send Hmm model from  master to slaves. It is 
										 // assumed that memory for Hmm is not allocated on slaves

extern void scatter_intVectors(int **u, int ***u_local, int ntotal, 
		int nlocal, int dim, int *nperproc); // scatter vectors of integers from master process to slaves 
		
extern void scatter_doubleVectors(double **u, double ***u_local, int ntotal, 
		int nlocal, int dim, int *nperproc); // scatter vectors of doubles from master process to slaves 		
		
		
extern void gather_intVectors(int ***u, int **u_local, int ntotal, 
		int nlocal, int dim, int *nperproc); // gather vectors of integers from slaves to master process
